...
%.pg
%
\item[\bb{Monks}]% are aesthetics, who by rigorous practice of physical and mental
...
%.pg
%
\item[\bb{Rangers}]% are most at home in the woods, and some say slightly out
...
%
\blist{}
\item[\bb{Dwarves}]% are smaller than humans or elves, but are stocky and solid
...
mitheral armor of the Elves.
%.pg
%
\item[\bb{Elves}]% are agile, quick, and perceptive; very little of what goes
on will escape an Elf.  The quality of Elven craftsmanship often gives
them an advantage in arms and armor.
%.pg
%
\item[\bb{Gnomes}]% are smaller than but generally similar to dwarves.  Gnomes are
...
%.pg
%
\item[\bb{Orcs}]% are a cruel and barbaric race, that hate every living thing
...
taken as good and Chaotic is evil.  Your alignment influences how other
...
used commands.  You may obtain a list of them by entering `{\tt ?}' after
the `{\tt \#}'.  What extended commands are available depends on what features
...
choose what you'll say), but chatting with some monsters such as the
Oracle of Delphi can produce useful results.
...
wild.  Simiarly, when you trigger certain types of traps which alter
...
has had resort to magic and wizardry in order to forge the alliance.
...
you would move yourself. It is the beast that you will see.
...
to your load.  The amount that you can carry depends on your strength.  The
...
with you through the dungeon will begin to encumber you.  Your reactions
will get slower and you'll burn calories faster, requiring food more frequentl
y
to cope with it.  Eventually, you'll be so overloaded that you'll either have
to discard some of what you're carrying or collapse under it's weight.
%.pg
...
%.pg
The ``{\tt \#enhance}'' command deals with your
proficiency in various types of weapons (also spells).
...
the ``{\tt \#enhance}'' extended command.    The `{\tt Q}' (quiver) and
`{\tt f}' (fire) commands are convenient for throwing a stack of objects
(such as arrows).  You may use two-weapon combat with the `{\tt x}'
(exchange) and ``{\tt \#twoweapon}'' commands.
...
dwarvish mitheril-coat & 4 &                   & elven mithril-coat    & 5\\
...
some rather unpleasent side-effects.
...
It is keystroke compatible with the traditional interface,
and is implemented only by the tty port (default off), when the game
has been compiled to support tty graphics.
%.lp
\item[\ib{female}]
...
%.lp
\item[\ib{horsename}]
Name your starting dog (ex.\ ``{\tt horsename:Trigger}'').
Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{IBMgraphics}]
...
Cannot be set with the `{\tt O}' command.
%.Ip
\item[\ib{pickup_burden}]
When you pick up an item that would exceeds this encumberance
...
%.lp
\item[\ib{pushweapon}]
When you use the `w' (wield) command and you had been wielding
something, place the old item in your secondary weapon slot (default off).
%.Ip
\item[\ib{race}]
...
value is examined; the string ``{\tt random}'' is an exception.
%.lp
\item[\ib{safe\_pet}]
...
was diagnosed with cancer and passed away.  This release of the game was
dedicated to him by the development and porting teams.

. pg
%.pg
\medskip
Late in 1999, the dev team released NetHack 3.3.  Along with the usual set
of bug fixes and minor tweaks (including Y2K proofing), a merge of features
was added to the game from variants that split off from the 3.2 base code.

%.pg
\medskip
During the lifespan of NetHack 3.2, Several enthusiasts of the game added
their own modifications to the game and made these "variants" publically
available:

%.pg
\medskip
{\it Tom Proudfoot} and {\it Yuval Oren} created NetHack++, which was later renamed
NetHack--.  Working independently, {\it Stephen White} wrote NetHack Plus.
{\it Tom Proudfoot} later merged NetHack Plus and his own NetHack-- to produce
SLASH.  {\it Larry Stewart-Zerba} and {\it Warwick Allison} improved the spell
casting system with the Wizard Patch.
  .pg
%.pg
\medskip
{\it Warren Cheung} combined SLASH with the Wizard Patch to produce Slash'em, and
with the help of {\it Kevin Hugo}, added more features.  Kevin later joined the
...
%.pg
\medskip
\nd {it Michael Allison} ported NetHack 3.3 for the MS-DOS platform.
...
%.pg
\medskip
\nd {it Michael Allison} ported 3.3 for the Microsoft Windows NT platform.

%.pg
\medskip
\nd {it Timo Hakulinen} continues to be responsible for the OS/2 port.
...
\begin{tabular}{lll}
%TABLE_START
Andy Church & Janet Walz & Mike Gallop\\
Andy Swanson & Jean-Christophe Collet & Mike Passaretti\\
Ari Huttunen & Jochen Erwied & Mike Stephenson\\
Barton House & John Kallen & Nick Number\\
Benson I. Margulies & John Rupley & Norm Meluch\\
Bill Dyer & John S. Bien & Olaf Seibert\\
Boudewijn Wayers & Johnny Lee & Pat Rankin\\
Bruce Holloway & Jon W\{tte & Paul Winner\\
Bruce Mewborne & Jonathan Handler & Pierre Martineau\\
Carl Schelin & Joshua Delahunty & Ralf Brown\\
David Cohrs & Keizo Yamamoto & Richard Addison\\
David Gentzel & Ken Arromdee & Richard P. Hughey\\
David Hairston & Ken Lorber & Rob Menke\\
Dean Luick & Ken Washikita & Roland McGrath\\
Del Lamb & Kevin Darcy & Ross Brown\\
Deron Meranda & Kevin Hugo & Scott R. Turner\\
Eric Backus & Kevin Sitze & Stephen Spackman\\
Eric Hendrickson & Kevin Smolkowski & Stephen White\\
Eric R. Smith & Kevin Sweet & Steve Creps\\
Eric S. Raymond & Mark Gooderum & Steve Linhart\\
Frederick Roeber & Mark Modrall & Steve VanDevender\\
Gil Neiger & Matthew Day & Tim Lennan\\
Greg Laskin & Merlyn LeRoy & Timo Hakulinen\\
Greg Olson & Michael Allison & Tom Almy\\
