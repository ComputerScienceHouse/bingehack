...
dosave0()
{
	...
	bwrite((genericptr_t) plname, PL_NSIZ, "char");
#endif
	...
	for(ltmp = (xchar)1; ltmp <= maxledgerno(); ltmp++) {
		...
		(void) close(ofd);
		bwrite((genericptr_t) &ltmp, sizeof ltmp, "char"); /* level number*/
...
register int fd, mode;
{
...
#endif
	uid = getuid();
	bwrite((genericptr_t) &uid, sizeof uid, "int");
	bwrite((genericptr_t) &flags, sizeof(struct flag), "flag");
	bwrite((genericptr_t) &u, sizeof(struct you), "you");
	...
	bwrite((genericptr_t) &monstermoves, sizeof monstermoves, "long");
...
#ifdef STEED
	if(usteed_id)
	    bwrite((genericptr_t) &usteed_id, sizeof usteed_id, "int");
#endif
	bwrite((genericptr_t) pl_character, sizeof pl_character, "char");
	bwrite((genericptr_t) pl_fruit, sizeof pl_fruit, "char");
	bwrite((genericptr_t) &current_fruit, sizeof current_fruit, "int");
	savefruitchn(fd, mode);
	savenames(fd, mode);
	save_waterlevel(fd, mode);

#ifdef RECORD_ACHIEVE
        bwrite((genericptr_t) &achieve, sizeof achieve, "u_achieve");
#endif
#if defined(RECORD_REALTIME) || defined(REALTIME_ON_BOTL)
...
/*#ifdef TOS
...
#ifdef RLECOMP
	{
		...
		for (x = 0; x < COLNO; x++) {
		    ...
		    writeout:
			bwrite((genericptr_t)&match, sizeof(uchar), "char");
			bwrite((genericptr_t)rgrm, sizeof(struct rm), "rm");
...
#else
	bwrite((genericptr_t) levl,sizeof(levl), "rm");
#endif /* RLECOMP */

	bwrite((genericptr_t) &monstermoves,sizeof(monstermoves), "long");
	bwrite((genericptr_t) &upstair,sizeof(stairway), "stairway");
	bwrite((genericptr_t) &dnstair,sizeof(stairway), "stairway");
	bwrite((genericptr_t) &upladder,sizeof(stairway), "stairway");
	bwrite((genericptr_t) &dnladder,sizeof(stairway), "stairway");
	bwrite((genericptr_t) &sstairs,sizeof(stairway), "stairway");
	bwrite((genericptr_t) &updest,sizeof(dest_area), "dest_area");
	bwrite((genericptr_t) &dndest,sizeof(dest_area), "dest_area");
	bwrite((genericptr_t) &level.flags,sizeof(level.flags), "levelflags");
	bwrite((genericptr_t) doors, sizeof(doors), "mkroom");
...
void bwrite(genericptr_t loc, unsigned int num, const char* structname) {
	...
	failed = (printf("%s\n", structname) == -1);
	for (int i = num; i != 0 && failed == false; i--) 
		failed = (printf("%0x", (num - i) + loc) == -1);
...
register int fd, mode;
{
	...
	if (perform_bwrite(mode))
	    bwrite((genericptr_t) &cnt, sizeof(int), "int");

	for (tmplev = sp_levchn; tmplev; tmplev = tmplev2) {
	    tmplev2 = tmplev->next;
	    if (perform_bwrite(mode))
		bwrite((genericptr_t) tmplev, sizeof(s_level), "s_level");
	    if (release_data(mode))
...
register int fd, mode;
{
	...
	for (tmp_dam = damageptr; tmp_dam; tmp_dam = tmp_dam->next)
	    xl++;
	if (perform_bwrite(mode))
	    bwrite((genericptr_t) &xl, sizeof(xl), "int");
...
register struct obj *otmp;
{
	...
	while(otmp) {
		...
		bwrite((genericptr_t) &xl, sizeof(int), "int");
		bwrite((genericptr_t) otmp, xl + sizeof(struct obj), "obj");
	    }
	...
	if (perform_bwrite(mode))
	    bwrite((genericptr_t) &minusone, sizeof(int), "int");
}
...
register struct monst *mtmp;
{
	...
	if (perform_bwrite(mode))
	    bwrite((genericptr_t) &monbegin, sizeof(monbegin), "permonst");
	...
	while (mtmp) {
		...
		bwrite((genericptr_t) &xl, sizeof(int), "int");
		bwrite((genericptr_t) mtmp, xl + sizeof(struct monst), "monst");
	    }
	    if (mtmp->minvent)
...
register struct trap *trap;
{
	...
	while (trap) {
	    trap2 = trap->ntrap;
	    if (perform_bwrite(mode))
		bwrite((genericptr_t) trap, sizeof(struct trap), "trap");
	    if (release_data(mode))
		dealloc_trap(trap);
	    trap = trap2;
	}
	if (perform_bwrite(mode))
	    bwrite((genericptr_t)nulls, sizeof(struct trap), "trap");
}
...
register int fd, mode;
{
	...
	while (f1) {
		...
		bwrite((genericptr_t) f1, sizeof(struct fruit), "fruit");
	    if (release_data(mode))
		dealloc_fruit(f1);
	    f1 = f2;
	}
	if (perform_bwrite(mode))
	    bwrite((genericptr_t)nulls, sizeof(struct fruit), "fruit");
	if (release_data(mode))
	    ffruit = 0;
}
