...
struct monst* mon;
{
  ...
  if (reg->max_monst <= reg->n_monst) {
    tmp_m = (unsigned int*) alloc(sizeof(unsigned int) * 
...
int mode;
{
  ...
  for (i =0; i < n_regions; i++) {
    ...
    n = 0;
    bwrite(fd, (genericptr_t) &regions[i]->attach_2_m, sizeof(unsigned int));
    n = regions[i]->enter_msg != NULL ? strlen(regions[i]->enter_msg) : 0;
    bwrite(fd, (genericptr_t) &n, sizeof(unsigned int));
    if (n > 0)
    ...
    for (j = 0; j < regions[i]->n_monst; j++)
      bwrite(fd, (genericptr_t) &regions[i]->monsters[j], sizeof(unsigned int));
...
int fd;
{
  ...
  for (i = 0; i < n_regions; i++) {
    ...
    if (regions[i]->nrects > 0)
      regions[i]->rects = (NhRect*) alloc(sizeof(NhRect) * regions[i]->nrects);
    for (j = 0; j < regions[i]->nrects; j++)
      mread(fd, (genericptr_t) &regions[i]->rects[j], sizeof(NhRect));
    mread(fd, (genericptr_t) &regions[i]->attach_2_u, sizeof(boolean));
    mread(fd, (genericptr_t) &regions[i]->attach_2_m, sizeof(unsigned int));

    mread(fd, (genericptr_t) &n, sizeof(unsigned int));
    if (n > 0) {
    ...
    if (regions[i]->ttl >= 0)
      regions[i]->ttl = regions[i]->ttl > tmstamp ? regions[i]->ttl - tmstamp  : 0;
    ...
    if (regions[i]->n_monst > 0)
      regions[i]->monsters = (unsigned int*) alloc(sizeof(unsigned int) * 
			regions[i]->n_monst);
    else
      regions[i]->monsters = NULL;
    regions[i]->max_monst = regions[i]->n_monst;
    for (j = 0; j < regions[i]->n_monst; j++)
      mread(fd, (genericptr_t) &regions[i]->monsters[j], sizeof(unsigned int));
...
genericptr_t p2;
{
  ...
  if (p2 == NULL) {		/* That means the player */
	...
	You("bump into some kind of invisible wall. Ouch!");
    else
      pline("Ouch!");
  } else {
    mtmp = (struct monst*) p2;
    if (canseemon(mtmp))
      pline("%s bump into something!", Monnam(mtmp));
  }
  return FALSE;
}
