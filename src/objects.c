...
#define TFHAT(name,desc,wt,ac,metal,c) \
	HELM(name,desc,0,0,0,1,1,wt,100,ac,0,metal,c)
...
/* TF2 Hats 09/29/11 */
/* Scout */
TFHAT("batter's helmet", "baseball helmet", 
	    10, 9, PLASTIC, CLR_RED),
TFHAT("bonk helm", "hat with cans stuck to it",
	    11, 9, PLASTIC, CLR_RED),
TFHAT("ye olde baker boy", "old hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("troublemaker's tossle cap", "beanie",
	    1, 10, CLOTH, CLR_RED),
TFHAT("whoopee cap", "cloth crown",
	    5, 10, CLOTH, CLR_RED),
TFHAT("milkman", "milk deliverer hat",
	    5, 10, CLOTH, CLR_WHITE),
TFHAT("bombing run", "old aviator hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("flipped trilby", "old pilgrim hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("superfan", "baseball cap",
	    2, 10, CLOTH, CLR_GREEN),
TFHAT("el jefe", "rebel's hat",
	    5, 10, CLOTH, CLR_GREEN),
/* Soldier */
TFHAT("soldier's stash", "army hat with smokes",
	    10, 9, IRON, CLR_BROWN),
TFHAT("stainless pot", "un-dented pot",
	    10, 9, IRON, CLR_BLACK),
TFHAT("tyrant's helm", "norse helm",
	    30, 9, IRON, CLR_RED),
TFHAT("killer's kabuto", "japanese helm",
	    30, 9, IRON, CLR_RED),
TFHAT("sergeant's drill hat", "boot camp hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("lumbricus lid", "army hat with golden grenades",
	    10, 9, IRON, CLR_YELLOW),
TFHAT("grenadier's softcap", "old army hat",
	    5, 10, CLOTH, CLR_BLUE),
TFHAT("chieftain's challenge", "native amerian hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("dr's dapper topper", "peculiar hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("stout shako", "marching hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("defiant spartan", "romain helm",
	    30, 8, IRON, CLR_YELLOW),
TFHAT("team captain", "army hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("furious fukaamigasa", "japanese hat",
	    5, 10, CLOTH, CLR_YELLOW),
TFHAT("stahlhelm", "old army hat",
	    10, 9, IRON, CLR_BROWN),
TFHAT("armored authority", "old army hat",
	    10, 9, IRON, CLR_BLACK),
TFHAT("jumper's jeepcap", "old driver hat",
	    5, 10, CLOTH, CLR_GREEN),
TFHAT("lord cockswain's pith helmet", "safari hat",
	    5, 10, CLOTH, CLR_YELLOW),
/* Pyro */
TFHAT("pyro's beanie", "beanie",
	    1, 10, CLOTH, CLR_RED),
TFHAT("brigade helm", "fireman helm",
	    10, 9, IRON, CLR_RED),
TFHAT("respectless rubber glove", "rubber glove",
	    1, 10, PLASTIC, CLR_YELLOW),
TFHAT("triboniophorus tyrannus", "frendly goo creature",
	    1, 10, FLESH, CLR_GREEN),
TFHAT("vintage merryweather", "old army helm",
	    30, 9, IRON, CLR_BLACK),
TFHAT("attendant", "old hat",
	    5, 10, CLOTH, CLR_GRAY),
TFHAT("handyman's handle", "plunger",
	    5, 10, WOOD, CLR_RED),
TFHAT("napper's respite", "bedtime hat",
	    1, 10, CLOTH, CLR_RED),
TFHAT("old guadalajara", "mexican hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("foster's facade", "gas mask",
	    10, 10, CLOTH, CLR_GRAY),
TFHAT("madame dixie", "high class lady hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("pyromancer's mask", "goat skull",
	    10, 10, BONE, CLR_GRAY),
TFHAT("prancer's pride", "a single antler",
	    5, 10, BONE, CLR_GRAY),
TFHAT("dead cone", "traffic cone",
	    10, 10, PLASTIC, CLR_ORANGE),
TFHAT("hottie's hoodie", "hoodie",
	    5, 10, CLOTH, CLR_RED),
TFHAT("connoisseur's cap", "chef hat",
	    5, 10, CLOTH, CLR_WHITE),
TFHAT("stately steel toe", "shoe",
	    5, 10, CLOTH, CLR_BROWN),
/* Demoman */
TFHAT("glengarry bonnet", "scottish hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("scotsman's stove pipe", "fancy hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("hustler's hallmark", "pimp hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("tippler's tricorne", "old pilgrim hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("carouser's capotain", "old pilgrim hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("rimmed raincatcher", "old hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("sober stuntman", "stunt helmet",
	    10, 9, IRON, CLR_WHITE),
TFHAT("scotch bonnet", "police helmet",
	    10, 9, IRON, CLR_BLACK),
TFHAT("prince tavish's crown", "crown",
	    10, 10, IRON, CLR_YELLOW),
TFHAT("Samur-Eye", "japanese helm",
	    30, 9, IRON, CLR_BLACK),
TFHAT("private eye", "detective hat",
	    3, 10, CLOTH, CLR_BROWN),
TFHAT("conjurer's cowl", "cowl",
	    5, 10, CLOTH, CLR_RED),
TFHAT("sultan's ceremonial", "sultan hat",
	    10, 10, CLOTH, CLR_RED),
TFHAT("tam o' shanter", "cap",
	    3, 10, CLOTH, CLR_RED),
/* Heavy */
TFHAT("football helmet‎‎", (char *)0,
	    10, 9, IRON, CLR_RED),
TFHAT("officer's ushanka", "russian army hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("tough guy's toque", "beanie",
	    3, 10, CLOTH, CLR_RED),
TFHAT("heavy duty rag", "durag",
	    1, 10, CLOTH, CLR_RED),
TFHAT("hard counter", "head unbrella",
	    5, 10, PLASTIC, CLR_RED),
TFHAT("pugilist's protector", "boxer hat",
	    5, 9, CLOTH, CLR_RED),
TFHAT("dealer's visor", "visor",
	    3, 10, CLOTH, CLR_GREEN),
TFHAT("big chief", "native american hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("magnificent mongolian", "mongolian hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("capo's capper", "old hat",
	    3, 10, CLOTH, CLR_BLACK),
TFHAT("copper's hard top", "police helmet",
	    10, 9, IRON, CLR_WHITE),
TFHAT("pilotka", "russian hat",
	    10, 10, CLOTH, CLR_BROWN),
TFHAT("dragonborn helmet", "goat skull",
	    10, 9, BONE, CLR_BROWN),
TFHAT("storm spirit's jolly hat", "japanese hat",
	    5, 10, CLOTH, CLR_RED),
/* Engineer */
TFHAT("mining light", "miner hat",
	    20, 9, IRON, CLR_ORANGE),
TFHAT("engineer's cap", "conductor hat",
	    5, 10, CLOTH, CLR_RED),
TFHAT("texas ten gallon", "texan hat",
	    5, 10, CLOTH, CLR_WHITE),
TFHAT("buckaroos hat", "cowboy hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("hetman's headpiece", "irsh hat",
	    5, 10, CLOTH, CLR_BLACK),
TFHAT("western wear", "western hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("ol' geezer", "old hat",
	    5, 10, CLOTH, CLR_BROWN),
/* Medic */
TFHAT("prussian pickelhaube", "german helm",
	    20, 9, IRON, CLR_BROWN),
TFHAT("vintage tyrolean", "old german hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("gentleman's gatsby", "german hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("berliner's bucket helm", "iron helm",
	    20, 9, IRON, CLR_BLACK),
TFHAT("german gonzila", "german hat",
	    3, 10, CLOTH, CLR_WHITE),
TFHAT("medic's mountain cap‎", "cap",
	    3, 10, CLOTH, CLR_BROWN),
TFHAT("grimm hatte‎", "old hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("doctor's sack", "ice pack",
	    5, 10, CLOTH, CLR_WHITE),
TFHAT("planeswalker helm", "horned helm",
	    20, 9, IRON, CLR_GREEN),
/* Sniper */
TFHAT("trophy belt", "tooth rimed hat",
	    5, 10, CLOTH, CLR_WHITE),
TFHAT("professional's panama", "hat",
	    5, 10, COTH, CLR_BROWN),
TFHAT("shooter's sola topi", "sola topi",
	    5, 10, CLOTH, CLR_GREEN),
TFHAT("bloke's bucket hat", "fishing hat",
	    5, 10, CLOTH, CLR_GREEN),
TFHAT("larrikin robin", "old hat",
	    5, 10, CLOTH, CLR_GREEN),
TFHAT("crocleather slouch", "leather hat",
	    10, 10, LEATHER, CLR_WHITE),
TFHAT("desert marauder", "marauder garb",
	    10, 10, CLOTH, CLR_WHITE),
TFHAT("anger", "hoodie",
	    5, 10, CLOTH, CLR_RED),
/* Spy */
TFHAT("backbiter's billycock", "old hat",
		3, 10, CLOTH, CLR_BROWN),
TFHAT("frenchman's beret", "beret",
	    3, 10, CLOTH, CLR_BROWN),
TFHAT("familiar fez", "fez",
	    3, 10, CLOTH, CLR_RED),
TFHAT("detective noir", "hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("charmer's chapeau‎", "charming hat",
	    5, 10, CLOTH, CLR_BROWN),
TFHAT("janissary ketche", "egyptian hat",
	    5, 10, CLOTH, CLR_YELLOW),
TFHAT("cosa nostra cap", "cap",
	    5, 10, CLOTH, CLR_RED),
TFHAT("nanobalaclava", "futuristic hat",
	    30, 8, IRON, CLR_BLACK),
/* suits of armor */
/*
